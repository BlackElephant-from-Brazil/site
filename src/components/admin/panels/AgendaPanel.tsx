...
function AddEntryForm({ userId, date, projects, onSaved, onCancel }: AddFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      ...
      {error && <p className="text-[0.7rem]" style={{ color: 'var(--color-error)' }}>{error}</p>}

      <div className="flex gap-2">
        <button type="submit" disabled={saving} className="flex-1 rounded-lg py-2 text-xs font-semibold" style={{ background: saving ? 'var(--card-border)' : 'var(--primary)', color: 'var(--primary-contrast)' }}>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="shrink-0 rounded-full px-2 py-0.5 text-[0.65rem] font-bold" style={{ background: 'rgba(31,111,107,0.12)', color: 'var(--primary)' }}>
        ...
        {HOUR_SLOTS.map(h => {
          ...
          return (
            <div key={h} className="flex gap-2" style={{ minHeight: hasEntries ? 'auto' : '32px' }}>
              ...
              <div className="flex-1 border-l pb-2 pl-2.5" style={{ borderColor: 'rgba(18,59,79,0.05)' }}>
                {hasEntries ? (
    ...
    <div className="flex h-full flex-col" style={{ width: '360px', borderLeft: '1px solid var(--card-border)', background: 'var(--nav-background)' }}>
      ...
      <div className="flex shrink-0 items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid var(--card-border)' }}>
        <div className="flex items-center gap-2">
          ...
          {totalMinutes > 0 && (
            <span className="rounded-full px-2 py-0.5 text-[0.65rem] font-bold" style={{ background: 'rgba(31,111,107,0.12)', color: 'var(--primary)' }}>
        ...
        <AnimatePresence initial={false}>
          {showForm === 'add' ? (
            ...
            <motion.button
              ...
              style={{ border: '1px dashed rgba(31,111,107,0.3)', color: 'var(--primary)', background: 'rgba(31,111,107,0.04)' }}
            >
